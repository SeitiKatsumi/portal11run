...
export const metadata: Metadata = {
  title: `${title} | Opinião 11RUN`, description,
...
export default function MaturationOpinionPage() {
  ...
  const schemas = [{ "@context": "https://schema.org", "@type": "Article", headline: title, description, author: { "@type": "Person", name: "Seiti Katsumi" }, publisher: { "@type": "Organization", name: "11RUN", url: origin }, mainEntityOfPage: `${origin}${path}` }, { "@context": "https://schema.org", "@type": "BreadcrumbList", itemListElement: [{ "@type": "ListItem", position: 1, name: "Início", item: origin }, { "@type": "ListItem", position: 2, name: "Institucional", item: `${origin}/institucional/missao-visao-valores` }, { "@type": "ListItem", position: 3, name: "Opinião", item: `${origin}${path}` }] }, { "@context": "https://schema.org", "@type": "FAQPage", mainEntity: faqs.map(([question, answer]) => ({ "@type": "Question", name: question, acceptedAnswer: { "@type": "Answer", text: answer } })) }];
  ...
  return <article className={styles.page}>
    ...
    <nav className={styles.breadcrumb} aria-label="Trilha de navegação"><Link href="/">Início</Link><span>/</span><Link href="/institucional/missao-visao-valores">Institucional</Link><span>/</span><strong>Opinião</strong></nav>
    <header className={styles.hero}>
      <div className={styles.heroCopy}><span className={styles.eyebrow}>Opinião 11RUN · Ciência aplicada ao esporte de base</span><h1>Mesma idade.<br /><em>Desenvolvimentos diferentes.</em></h1><p>No esporte de base, o cronômetro mostra o resultado de hoje — mas não define, sozinho, o potencial de amanhã.</p><div className={styles.actions}><a href="#entenda">Entenda como funciona <ArrowDown /></a><Link href="/onze-futuro">Conheça o Onze Futuro <ArrowRight /></Link></div><small>Ciência · Acompanhamento longitudinal · Infância protegida · Decisões humanas</small></div>
    ...
    <nav className={styles.jumpNav} aria-label="Nesta opinião">{[["Entenda","entenda"],["As diferenças","diferencas"],["Crescimento","crescimento"],["Talento","talento"],["Método 11RUN","metodo"],["Para famílias","familias"],["Ciência","ciencia"],["Dúvidas","duvidas"]].map(([label,id]) => <a key={id} href={`#${id}`}>{label}</a>)}</nav>
    <MaturationExperience />
    <section className={styles.signature}><div><span className={styles.eyebrow}>Opinião</span><h2>Desenvolver é continuar dando oportunidade para o talento aparecer.</h2><p>Resultados importam. Competições importam. Mas uma criança precisa ser vista como uma trajetória em construção — nunca como uma marca definitiva.</p></div><div><strong>Seiti Katsumi</strong><span>Fundador da 11RUN</span></div></section>
