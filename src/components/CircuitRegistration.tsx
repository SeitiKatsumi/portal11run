"use client";
import { CIRCUIT_TRACK_RULE, CIRCUIT_PRIZE_EVIDENCE_RULE } from "@/lib/virtual-circuit-schedule";

import { Check, ChevronLeft, ChevronRight, FileCheck2, HeartHandshake, Info, LoaderCircle, LockKeyhole, Send, ShieldAlert } from "lucide-react";
...
const initial: FormState = {
  ...
  activityState: "SP", mediaPromotion: false, medicalMethod: "GUARDIAN_COMMITMENT", medicalCommitment: false
};

function category(birthDate: string) {
  const year = Number(birthDate.slice(0, 4));
  const age = 2026 - year;
  return age >= 9 && age <= 13 ? circuitCategoryLabel(age) : "Fora das categorias Sub 10 a Sub 14 desta edição";
}
  ...
  function validateStep() {
    ...
    if (step === 2) {
      if (!data.medicalGuardianCpf) {
        setMessage({ type: "error", text: "Confirme o CPF do responsável para assinar o termo." }); return false;
      }
      if (data.medicalCommitment !== true) {
        setMessage({ type: "error", text: "Leia e aceite o termo de saúde e responsabilidade." }); return false;
      }
    }
    if (step === 4) {
  ...
  async function submit() {
    ...
    const payload = {
      ...
      consents: Object.fromEntries([
        ...consentItems.map(([key]) => [key, data[key] === true]),
        ["GUARDIAN_HEALTH_RESPONSIBILITY", data.medicalCommitment === true],
        ["MEDIA_PROMOTION", data.mediaPromotion === true]
      ]),
      meta: {}
    };
  ...
  return (
    <div className={styles.registration}>
      ...
      <div className={styles.formCard}>
        ...
        {step === 6 && <Review data={data} category={athleteCategory} fileName={fileName} startDate={startDate} />}
...
function StepGuardian({data,set}:StepProps){return <div><h3>Responsável legal e declaração de saúde</h3><div className={styles.fieldGrid}><Input label="Nome completo" name="guardianFullName" data={data} set={set}/><Input label="CPF" name="guardianCpf" data={data} set={set}/><Input label="Relação com o atleta" name="guardianRelationship" data={data} set={set}/><Input label="Data de nascimento" name="guardianBirthDate" data={data} set={set} type="date"/><Input label="E-mail" name="guardianEmail" data={data} set={set} type="email"/><Input label="WhatsApp" name="guardianPhone" data={data} set={set}/></div>
  <div className={styles.medicalSection}>
    ...
    <div className={styles.safetyNotice}><ShieldAlert/><span>A participação é virtual. A 11Run não escolhe nem fiscaliza presencialmente o local ou a execução do teste. O aceite registra a responsabilidade do responsável legal, sem afastar direitos e deveres que não possam ser excluídos por lei.</span></div>
  </div>
...
function StepType({data,set}:StepProps){return <div><h3>Como esta marca foi obtida?</h3><div className={styles.typeCards}>{[["OFFICIAL_COMPETITION","Competição oficial","Resultado público de federação ou organização."],["TRACK_400M","Pista oficial de 400m","2,5 voltas oficiais, sem medição por GPS e com vídeo integral."],["OPEN_COURSE","Percurso aberto","Atividade no Strava, vídeo e altimetria."]].map(([value,title,text])=><button type="button" key={value} className={data.type===value?styles.selected:""} onClick={()=>set("type",value)}><FileCheck2/><strong>{title}</strong><span>{text}</span></button>)}</div></div>}
