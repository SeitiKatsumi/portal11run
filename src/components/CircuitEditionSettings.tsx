...
export function CircuitEditionSettings({initialEdition}:{initialEdition:Edition}){
  ...
  async function save(){setBusy(true);setMessage("Salvando…");try{const response=await fetch("/api/admin/circuito-virtual/edition",{method:"PATCH",signal:AbortSignal.timeout(20000),headers:{"Content-Type":"application/json"},body:JSON.stringify({startDate,endDate,heroImage:hero,regulations:fromLines(regulations),faq:fromLines(faq),settings:{...initialEdition.settings,elevationToleranceMeters:Number(tolerance)}})});const result=await response.json();setMessage(response.ok?"Configurações salvas.":result.error||"Falha ao salvar.");}catch{setMessage("Falha de conexão. Tente salvar novamente.");}finally{setBusy(false);}}
  return <section className={`admin-panel ${styles.settings}`}><button className={styles.trigger} aria-expanded={open} aria-controls="circuit-edition-settings" onClick={()=>setOpen(value=>!value)}><Settings2 size={18}/><span><strong>Configuração da edição 2026</strong><small>Datas, imagem, altimetria, regulamento e FAQ</small></span><b>{open?"Fechar":"Editar"}</b></button>{open&&<div id="circuit-edition-settings" className={styles.body}><div className={styles.grid}><label>Início<input type="date" value={startDate} onChange={e=>setStart(e.target.value)}/></label><label>Encerramento<input type="date" value={endDate} onChange={e=>setEnd(e.target.value)}/></label><label>Imagem do hero<input value={hero} onChange={e=>setHero(e.target.value)} placeholder="/assets/onze-futuro-hero.jpg"/></label><label>Tolerância de altimetria (m)<input type="number" min="0" max="20" value={tolerance} onChange={e=>setTolerance(e.target.value)}/></label></div><label>Regulamento <small>Uma regra por linha no formato Título | Texto</small><textarea rows={12} value={regulations} onChange={e=>setRegulations(e.target.value)}/></label><label>FAQ <small>Uma pergunta por linha no formato Pergunta | Resposta</small><textarea rows={10} value={faq} onChange={e=>setFaq(e.target.value)}/></label><div className={styles.footer}>{message&&<span role="status">{message}</span>}<button disabled={busy} onClick={save}><Save size={16}/>Salvar edição</button></div></div>}</section>
}
